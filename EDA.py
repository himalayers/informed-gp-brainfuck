...
def run(INPUTS, TARGETS, N=5, model={}, ALPHA=0.9, PROGRATIO=0.9, POPSIZE=100, GENNO=200, MAXITERS=500, XBOUNDS=[2,10], YBOUND=1, MAXPROGSIZE=150, verbose=-1):
    TERMINATEFIT = 1
    targetprog = BFAST2.parseBF(BFGeneratorForWords.wordToBF(TARGETS[0]))
    if verbose in [2]:
        print("target prog:", BFAST2.unparseBF(targetprog))

    #initialise population with model
    if model!={}:
